<template>
  <v-card>
    <v-container>
      ...
      <v-card-text>
        {{ applicant.self_introduction }}
      </v-card-text>

      <v-card-subtitle class="font-weight-bold pb-1 pt-0">
        -{{ $t('user.sex') }}-
      </v-card-subtitle>
      <v-card-text>
        {{ $t(`gender.${applicant.sex}`) }}
      </v-card-text>
      ...
      <v-card-text>
        {{ getUserAge(applicant.date_of_birth) }}歳
      </v-card-text>
...
<script>
export default {
  name: 'ApplicationApplicantProfile',
  props: {
    applicant: {
      type: Object,
      default: () => {},
